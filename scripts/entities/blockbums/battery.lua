...
local blockbumSubType = 5

local damageCooldown = 30
local slamCooldown = 120
local maxHP = 10
local maxRooms = 5
local gibsAmount = 5
local collDamage = 5
...
function funcs:familiarInit(familiar)
    if(familiar.SubType==blockbumSubType) then
        ...
        sprite:Load("gfx/entities/familiars/blockbum/battery.anm2", true)
...
function funcs:familiarUpdate(familiar)
    if(familiar.SubType==blockbumSubType) then
        ...
        if(sprite:IsFinished("Slam")) then
            if(data.numSlams==1) then sprite:Play("Jump", true)
            elseif(data.numSlams==2) then
