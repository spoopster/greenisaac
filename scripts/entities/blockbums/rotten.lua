...
local gibsAmount = 5
local collDamage = 3
local smokeColor = Color(0.5,0.8,0.4,0.8,0,0,0)
...
function funcs:familiarInit(familiar)
    if(familiar.SubType==blockbumSubType) then
        ...
        sprite:Load("gfx/entities/familiars/blockbum/rotten.anm2", true)
...
function funcs:familiarUpdate(familiar)
    if(familiar.SubType==blockbumSubType) then
        ...
        if(sprite:IsEventTriggered("Slam")) then
            ...
            if(#entities>0) then
                sfx:Play(damageSFX)
                familiar.Player:AddBlueFlies(2, familiar.Player.Position, nil)
            end
        ...
        if(familiar.FrameCount%3==0 and familiar.Color.RO==1) then
        ...
        if((anim=="Jump" and data.isTouchable==false) or anim=="JumpIdle" or (anim=="Slam" and data.isTouchable==false)) then
            local speedMult=0.15*(familiar.Coins/20)+0.005*(1-familiar.Coins/20)
            familiar.Coins = familiar.Coins+1
